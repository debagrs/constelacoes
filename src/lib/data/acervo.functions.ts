...
/**
 * Destaques da home: uma amostra nova e aleatória a cada chamada.
 * O subselect evita repetir exatamente a mesma URL de imagem no mesmo lote.
 */
        ...
        AND e.id IN (
          SELECT MIN(e2.id)
           ...
           WHERE e2.status = 'published'
             AND e2.image_url IS NOT NULL
             AND trim(e2.image_url) <> ''
           GROUP BY lower(trim(e2.image_url))
        )
      ORDER BY RANDOM()
      LIMIT 6`,
  );
