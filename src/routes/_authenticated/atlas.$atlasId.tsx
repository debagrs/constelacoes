...
function AtlasEditor() {
  ...
  const addEntityCard = (entity: SearchEntity) => {
    const entityMediaUrl = aicMetadataImageUrl(entity.metadata, 843) ?? entity.image_url;
    const alreadyAdded = cards.some(
      (card) => card.entity_id === entity.id || card.media_url === entityMediaUrl || card.media_url === entity.image_url,
    );
    if (alreadyAdded) {
              ...
              <section className="rounded-xl border border-border/60 bg-card p-4">
                ...
                <div className="mt-4 grid max-h-[520px] grid-cols-2 gap-3 overflow-y-auto pr-1 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6">
                  {searchResults.map((entity) => {
                    ...
                    const alreadyAdded = cards.some(
                      (card) =>
                        card.entity_id === entity.id || card.media_url === entityMediaUrl || card.media_url === entity.image_url,
                    );
                    return (
                      <button
                        type="button"
                      ...
                      >
                        <div className="aspect-[4/5] overflow-hidden bg-muted">
                          <AtlasLibraryImage entity={entity} />
                        </div>
                        <div className="p-2">
...
function AtlasLibraryImage({ entity }: { entity: SearchEntity }) {
  const initialCandidates = useMemo(() => {
    const values = [aicMetadataImageUrl(entity.metadata, 400), entity.image_url]
      .filter((value): value is string => Boolean(value?.trim()));
    return [...new Set(values)];
  }, [entity.image_url, entity.metadata]);
  ...
  const handleError = async () => {
    ...
    if (!liveTried && /^aic-\d+$/i.test(entity.id)) {
      setLiveTried(true);
      const live = await fetchCurrentAicImageUrl(entity.id, entity.source_url, 400);
      if (live && !candidates.includes(live)) {
        setCandidates((values) => [...values, live]);
  ...
  return current ? (
    ...
    <div className="flex h-full w-full items-center justify-center bg-muted text-[0.65rem] text-muted-foreground">
      imagem indisponível
    </div>
  );
}

function AtlasCardImage({ card, onResolved }: { card: CardRow; onResolved: (url: string) => void }) {
  const [src, setSrc] = useState(card.media_url ?? "");
  const [liveTried, setLiveTried] = useState(false);

  useEffect(() => {
    setSrc(card.media_url ?? "");
    setLiveTried(false);
  }, [card.entity_id, card.media_url]);

  const handleError = async () => {
    if (!liveTried && card.entity_id && /^aic-\d+$/i.test(card.entity_id)) {
      setLiveTried(true);
      const sourceUrl = card.link_url && /^https?:\/\//i.test(card.link_url) ? card.link_url : null;
      const live = await fetchCurrentAicImageUrl(card.entity_id, sourceUrl, 843);
      if (live && live !== src && live !== card.media_url) {
        setSrc(live);
        onResolved(live);
        return;
      }
                ...
                <div className="flex h-full flex-col overflow-hidden rounded-md">
                  <div className="min-h-0 flex-1 bg-muted">
                    <AtlasCardImage
                      card={card}
                      onResolved={(url) => {
                        if (url && url !== card.media_url) onChange(card.id, { media_url: url });
                      }}
                    />
                  </div>
