...
function AcervoPage() {
  ...
  function runExternalSearch() {
    const term = (externalQuery || query).trim();
          ...
          <section className="mb-12 rounded-2xl border border-primary/20 bg-primary/5 p-5 sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div className="max-w-2xl">
                <div className="flex items-center gap-2 text-primary">
                  ...
                  <h2 className="font-display text-2xl font-semibold">Acervos abertos do planeta</h2>
                </div>
                <p className="mt-2 text-sm text-muted-foreground">
                  Pesquise ao vivo no Metropolitan Museum, Art Institute of Chicago e Wikimedia Commons. As imagens permanecem nas instituições de origem; o Atlas apenas relaciona os links, créditos e licenças.
                </p>
              </div>
              <div className="flex w-full max-w-md gap-2">
                <Input
                  ...
                  onKeyDown={(event) => event.key === "Enter" && runExternalSearch()}
                  placeholder="Ex.: maternidade, indígena, animalidade..."
                />
                <Button onClick={runExternalSearch} disabled={externalSearch.isPending || (externalQuery || query).trim().length < 2}>
            ...
            {externalSearch.data?.errors?.length ? (
              <p className="mt-4 text-xs text-muted-foreground">
                Algumas fontes não responderam agora, mas as demais foram exibidas.
              </p>
            ) : null}
            ...
            {externalSearch.data?.results?.length ? (
              ...
              <p className="mt-6 text-sm text-muted-foreground">Nenhum resultado aberto foi encontrado para essa busca.</p>
            ) : null}
          </section>

          {/* Grid local curado no Turso */}
          {isLoading ? (
