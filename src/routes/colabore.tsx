...
function ColaborePage() {
  ...
  <form onSubmit={onSubmit} className="mt-10 space-y-10">
    ...
    <Section title="Imagem, licença e fontes"><ContributionImagePicker value={form.imageUrl} onChange={(v)=>set("imageUrl",v)}/><div className="grid gap-5 md:grid-cols-2"><Field label="URL direta da imagem (alternativa ao upload)" type="url" value={isInlineImage(form.imageUrl)?"":form.imageUrl} onChange={(v)=>set("imageUrl",v)}/><Field label="Página de origem da imagem" type="url" value={form.imageSourceUrl} onChange={(v)=>set("imageSourceUrl",v)}/><Field label="Licença" hint="Domínio público, CC BY, CC BY-SA…" value={form.imageLicense} onChange={(v)=>set("imageLicense",v)}/><Field label="Outras fontes" hint="URLs separadas por vírgulas" value={form.sourceUrls} onChange={(v)=>set("sourceUrls",v)}/></div></Section>
...
function ContributionImagePicker({value,onChange}:{value:string;onChange:(v:string)=>void}){
  ...
  return <div className="rounded-xl border border-dashed bg-muted/30 p-4 sm:p-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><Label htmlFor="contribution-image">Enviar uma foto</Label><p className="mt-1 text-xs text-muted-foreground">JPG, PNG, WEBP ou outra imagem. O arquivo é reduzido automaticamente antes do envio.</p></div>
      {(value&&inline)&&<Button type="button" size="sm" variant="outline" onClick={()=>onChange("")}><Trash2 className="mr-2 h-4 w-4"/>Remover foto</Button>}
    </div>
    ...
    {inline&&<div className="mt-4 overflow-hidden rounded-lg border bg-background"><img src={value} alt="Prévia da foto enviada" className="max-h-80 w-full object-contain"/></div>}
  </div>
}
