...
function MapItems({
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["map-items", regionId ?? null, continent ?? null, facet?.id ?? null, q, page, cursor],
    queryFn: () =>
      searchRegionItems({
        data: {
          id: regionId,
  ...
  function goNextPage() {
    if (!hasNext || !data?.nextCursor || isFetching) return;
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] tabular-nums text-muted-foreground">
            {total !== null ? `${total} ${total === 1 ? "item" : "itens"}` : `página ${page}`}
          </span>
        </div>
      </div>
