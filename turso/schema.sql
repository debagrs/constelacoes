...
-- Busca textual: o índice FTS nativo do Turso é criado de forma opcional por turso/apply-schema.mjs.
...
CREATE INDEX IF NOT EXISTS idx_entities_status_image ON entities(status, image_url);
CREATE INDEX IF NOT EXISTS idx_entities_status_region ON entities(status, region_id);
CREATE INDEX IF NOT EXISTS idx_entities_status_date ON entities(status, date_start);
CREATE INDEX IF NOT EXISTS idx_entities_status_country ON entities(status, country);
CREATE INDEX IF NOT EXISTS idx_entities_status_id ON entities(status, id);
CREATE INDEX IF NOT EXISTS idx_entities_status_region_id ON entities(status, region_id, id);
CREATE INDEX IF NOT EXISTS idx_entities_status_continent_id ON entities(status, continent, id);
CREATE INDEX IF NOT EXISTS idx_entities_status_type_id ON entities(status, entity_type, id);
CREATE INDEX IF NOT EXISTS idx_entities_status_updated ON entities(status, updated_at DESC, id);
CREATE INDEX IF NOT EXISTS idx_entities_title_nocase ON entities(title COLLATE NOCASE);
CREATE INDEX IF NOT EXISTS idx_entities_subtitle_nocase ON entities(subtitle COLLATE NOCASE);
CREATE INDEX IF NOT EXISTS idx_relations_status_source ON relations(status, source_id);
CREATE INDEX IF NOT EXISTS idx_relations_status_target ON relations(status, target_id);



------------------------------------------------------------------ camada planetária de baixo consumo
...
CREATE INDEX IF NOT EXISTS idx_regions_parent_sort ON regions(parent_id, sort_order, id);
...
CREATE INDEX IF NOT EXISTS idx_entity_facets_facet_entity ON entity_facets(facet_id, entity_id);
...
------------------------------------------------------------------ índices materiais / resumos de baixo consumo
-- Estes dados são reconstruídos por `npm run db:refresh-indexes` depois de cargas grandes.
-- As páginas públicas leem estas tabelas pequenas em vez de executar COUNT/GROUP BY no acervo inteiro.
...
CREATE TABLE IF NOT EXISTS atlas_type_stats (
...
CREATE TABLE IF NOT EXISTS atlas_facet_stats (
...
CREATE TABLE IF NOT EXISTS atlas_region_stats (
...
CREATE TABLE IF NOT EXISTS atlas_continent_stats (
...
CREATE TABLE IF NOT EXISTS atlas_region_timeline (
...
CREATE TABLE IF NOT EXISTS atlas_region_facet_stats (
  region_id TEXT NOT NULL REFERENCES regions(id) ON DELETE CASCADE,
  facet_id  TEXT NOT NULL REFERENCES facets(id) ON DELETE CASCADE,
  total     INTEGER NOT NULL DEFAULT 0,
  PRIMARY KEY (region_id, facet_id)
);
CREATE INDEX IF NOT EXISTS idx_region_facet_stats_region ON atlas_region_facet_stats(region_id, total DESC);

CREATE TABLE IF NOT EXISTS atlas_quality_issues (
  entity_id TEXT NOT NULL REFERENCES entities(id) ON DELETE CASCADE,
  issue     TEXT NOT NULL,
  PRIMARY KEY (entity_id, issue)
);
CREATE INDEX IF NOT EXISTS idx_atlas_quality_issues_issue ON atlas_quality_issues(issue, entity_id);

CREATE TABLE IF NOT EXISTS entity_facet_candidates (
  entity_id TEXT NOT NULL REFERENCES entities(id) ON DELETE CASCADE,
  facet_id  TEXT NOT NULL REFERENCES facets(id) ON DELETE CASCADE,
  evidence  TEXT NOT NULL DEFAULT '[]',
  PRIMARY KEY (entity_id, facet_id)
);
CREATE INDEX IF NOT EXISTS idx_entity_facet_candidates_facet ON entity_facet_candidates(facet_id, entity_id);

CREATE TABLE IF NOT EXISTS entity_dedupe_index (
  entity_id     TEXT PRIMARY KEY REFERENCES entities(id) ON DELETE CASCADE,
  image_key     TEXT,
  signature_key TEXT,
  is_canonical  INTEGER NOT NULL DEFAULT 1
);
CREATE INDEX IF NOT EXISTS idx_entity_dedupe_image ON entity_dedupe_index(image_key, is_canonical, entity_id);
CREATE INDEX IF NOT EXISTS idx_entity_dedupe_signature ON entity_dedupe_index(signature_key, is_canonical, entity_id);
CREATE INDEX IF NOT EXISTS idx_entity_dedupe_noncanonical ON entity_dedupe_index(is_canonical, image_key, signature_key);

CREATE TABLE IF NOT EXISTS atlas_import_state (
  source     TEXT PRIMARY KEY,
  cursor     TEXT NOT NULL DEFAULT '',
  updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now'))
);

CREATE TABLE IF NOT EXISTS atlas_capabilities (
  key        TEXT PRIMARY KEY,
