...
function facetsFromItem(item) {
  ...
  if (hasAny(text, TERMS.women)) facets.add(FACETS.women);
...
async function printStats(prefix) {
  const [published, uniqueImages, aicRecords, aicImages, aicWithoutImage] = await Promise.all([
    scalar(`SELECT COUNT(*) total FROM entities WHERE status='published'`),
    scalar(`SELECT COUNT(DISTINCT lower(trim(image_url))) total FROM entities WHERE status='published' AND image_url IS NOT NULL AND trim(image_url)<>''`),
    scalar(`SELECT COUNT(*) total FROM entities WHERE status='published' AND id LIKE 'aic-%'`),
    scalar(`SELECT COUNT(*) total FROM entities WHERE status='published' AND id LIKE 'aic-%' AND image_url IS NOT NULL AND trim(image_url)<>''`),
    scalar(`SELECT COUNT(*) total FROM entities WHERE status='published' AND id LIKE 'aic-%' AND (image_url IS NULL OR trim(image_url)='')`),
  ]);
  console.log(`\n${prefix}`);
  console.log(`Registros publicados no Atlas: ${published.toLocaleString('pt-BR')}`);
  console.log(`Imagens únicas no acervo visual: ${uniqueImages.toLocaleString('pt-BR')}`);
  console.log(`Registros AIC em domínio público: ${aicRecords.toLocaleString('pt-BR')}`);
  console.log(`AIC com imagem IIIF: ${aicImages.toLocaleString('pt-BR')}`);
  console.log(`AIC sem imagem disponível: ${aicWithoutImage.toLocaleString('pt-BR')}`);
}
...
async function main() {
  ...
  await printStats('ANTES DA SINCRONIZAÇÃO');
  ...
  if (RESET_CURSOR || cursor >= files.length) cursor = 0;
  ...
  await printStats('DEPOIS DA SINCRONIZAÇÃO');
